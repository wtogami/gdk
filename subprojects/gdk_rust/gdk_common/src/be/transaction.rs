...
impl BETransaction {
    ...
    pub fn estimated_fee(&self, fee_rate: f64) -> u64 {
        let mut fee_val = match self  {
            // last output is not consider for fee dynamic calculation, consider it here
            BETransaction::Bitcoin(_) => (70.0 * fee_rate) as u64,
            BETransaction::Elements(_) => (1200.0 * fee_rate) as u64,
        };

        let calc_fee_bytes = |bytes| ((bytes as f64) * fee_rate) as u64;
        fee_val += calc_fee_bytes(self.get_weight() / 4);

        //TODO should estimate how many changes are needed
    ...
    pub fn needs(
        ...
        // TODO increase needs of bitcoin and liquid bitcoin of the expected fee for changes also
        match self {
            Self::Bitcoin(tx) => {
                let sum_inputs= sum_inputs(tx, all_txs);
                let sum_outputs: u64 = tx.output.iter().map(|o| o.value).sum();
                let estimated_fee = self.estimated_fee(fee_rate);
            ...
            Self::Elements(tx) => {
                ...
                let estimated_fee = self.estimated_fee(fee_rate);
                *outputs.entry(policy_asset.unwrap()).or_insert(0) += estimated_fee;

                let mut inputs: HashMap<String, u64> = HashMap::new();
                for input in tx.input.iter() {
                    let asset_hex = all_txs.get_previous_output_asset_hex(input.previous_output, all_unblinded).unwrap();
                    let value = all_txs.get_previous_output_value(&BEOutPoint::Elements(input.previous_output), all_unblinded).unwrap();
                    *inputs.entry(asset_hex).or_insert(0) += value;
                }
                let mut result = vec![];
                for (asset,value) in outputs.iter() {
                    let sum = value - inputs.remove(asset).unwrap_or(0);
                    if sum > 0 {
                        result.push(AssetValue::new(asset.to_string(), sum));
                    }
                }
                assert!(inputs.is_empty());
                // TODO sort such that policy asset is the last
                result
            }
        }
    }
